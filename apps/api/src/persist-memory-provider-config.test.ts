...
describe("persistMemoryProviderConfig", () => {
  ...
  it("accepts an operator-allowlisted internal origin in local mode", async () => {
    vi.stubEnv("RAKAZO_SUPERMEMORY_LOCAL_ORIGINS", "http://supermemory:6767");
    const fetchMock = vi.fn().mockResolvedValue(new Response("[]", { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    const { deps, upsert } = makeDeps();
    ...
    await persistMemoryProviderConfig(
      deps as never,
      actor,
      connectionInput("local", "http://supermemory:6767"),
    );

    expect(fetchMock.mock.calls[0]![0]).toBe("http://supermemory:6767/v3/container-tags/list");
    expect(upsert).toHaveBeenCalled();
  });

  it("does not allow another port on an operator-allowlisted host", async () => {
    vi.stubEnv("RAKAZO_SUPERMEMORY_LOCAL_ORIGINS", "http://supermemory:6767");
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const { deps, upsert } = makeDeps();
    ...
    await expect(
      persistMemoryProviderConfig(
        deps as never,
        actor,
        connectionInput("local", "http://supermemory:6768"),
      ),
    ).rejects.toThrow(/RAKAZO_SUPERMEMORY_LOCAL_ORIGINS/);
    expect(fetchMock).not.toHaveBeenCalled();
    expect(upsert).not.toHaveBeenCalled();
  });

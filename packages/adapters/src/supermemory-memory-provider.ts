...
function configuredLocalOrigins(): Set<string> {
  const origins = new Set<string>();
  for (const value of (process.env.RAKAZO_SUPERMEMORY_LOCAL_ORIGINS ?? "").split(",")) {
    ...
    try {
      ...
      // Invalid allowlist entries never widen access.
    }
  }
  return origins;
}

function isAllowedLocalBaseUrl(url: string): boolean {
  try {
    ...
    return configuredLocalOrigins().has(parsed.origin);
  } catch {
    return false;
  }
}
...
function parseSupermemoryConnection(
  ...
  if (mode === "local" && !isAllowedLocalBaseUrl(baseUrl)) {
    throw new Error(
      "Local mode requires a loopback address or an origin listed in RAKAZO_SUPERMEMORY_LOCAL_ORIGINS.",
    );
  }
